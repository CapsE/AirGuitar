...
# define TRACE(fmt, ...) qDebug() << fmt << "\n"
# define WARN(fmt, ...) qDebug() << fmt << "\n"
...
static void _cdecl _TRACE (const TCHAR* fmt, ...)
    ...
    _vsntprintf_s((char*)buffer, ARRAY_ENTRIES(buffer), _TRUNCATE, (char*)fmt, argptr);
#else
...
const TCHAR*   wiimote::ButtonNameFromBit         [TOTAL_BUTTON_BITS] =
								{ (const TCHAR*)_T("Left"), (const TCHAR*)_T("Right"), (const TCHAR*)_T("Down"), (const TCHAR*)_T("Up"),
                                  (const TCHAR*)_T("Plus") , (const TCHAR*)_T("??")   , (const TCHAR*)_T("??")  , (const TCHAR*)_T("??") ,
                                  (const TCHAR*)_T("Two")  , (const TCHAR*)_T("One")  , (const TCHAR*)_T("B")   , (const TCHAR*)_T("A") ,
                                  (const TCHAR*)_T("Minus"), (const TCHAR*)_T("??")   , (const TCHAR*)_T("??")  , (const TCHAR*)_T("Home") };

const TCHAR*   wiimote::ClassicButtonNameFromBit [TOTAL_BUTTON_BITS] =
                                { (const TCHAR*)_T("??")   , (const TCHAR*)_T("TrigR")  , (const TCHAR*)_T("Plus") , (const TCHAR*)_T("Home"),
                                  (const TCHAR*)_T("Minus"), (const TCHAR*)_T("TrigL") , (const TCHAR*)_T("Down") , (const TCHAR*)_T("Right") ,
                                  (const TCHAR*)_T("Up")   , (const TCHAR*)_T("Left")   , (const TCHAR*)_T("ZR")   , (const TCHAR*)_T("X") ,
                                  (const TCHAR*)_T("A")    , (const TCHAR*)_T("Y")      , (const TCHAR*)_T("B")    , (const TCHAR*)_T("ZL") };
// ------------------------------------------------------------------------------------
wiimote::wiimote ()
    :
    ...
    if(++_TotalCreated == 1)
        {
			HidDLL = LoadLibraryA(_T("hid.dll"));
        _ASSERT(HidDLL);
        if(!HidDLL)
...
#ifdef TRACE
#define TYPE2NAME(_type)    (type==_type)? (const TCHAR*)_T(#_type)
    const TCHAR* name = TYPE2NAME(IN_BUTTONS)                :
                        ...
                        (const TCHAR*)_T("(unknown??)");
	TRACE((const TCHAR*)_T("ReportType: %s (%s)"), name, (continuous ? (const TCHAR*)_T("continuous") :
		(const TCHAR*)_T("non-continuous")));
#endif
    ReportType = type;
...
int wiimote::ParseAccel (BYTE* buff)
    ...
    BYTE raw_x_LSB = (buff[1] & 96) >> 5;
    BYTE raw_y_LSB = (buff[2] & 32) >> 5;
    BYTE raw_z_LSB = (buff[2] & 64) >> 6;

    if((raw_x != Internal.Acceleration.RawX)         ||
       (raw_x_LSB != Internal.Acceleration.RawXLSB)  ||
       (raw_y != Internal.Acceleration.RawY)         ||
       (raw_y_LSB != Internal.Acceleration.RawYLSB)  ||
       (raw_z != Internal.Acceleration.RawZ)         ||
       (raw_z_LSB != Internal.Acceleration.RawZLSB))
    ...
    Internal.Acceleration.RawXLSB = raw_x_LSB;
    Internal.Acceleration.RawYLSB = raw_y_LSB;
    Internal.Acceleration.RawZLSB = raw_z_LSB;

    // avoid / 0.0 when calibration data hasn't arrived yet
    if(Internal.CalibrationInfo.X0)
        {
        Internal.Acceleration.X =
                    ((float)Internal.Acceleration.RawX  + (float)Internal.Acceleration.RawXLSB/4.0f - Internal.CalibrationInfo.X0) /
                    ((float)Internal.CalibrationInfo.XG - Internal.CalibrationInfo.X0);
        Internal.Acceleration.Y =
                    ((float)Internal.Acceleration.RawY  + (float)Internal.Acceleration.RawYLSB/2.0f - Internal.CalibrationInfo.Y0) /
                    ((float)Internal.CalibrationInfo.YG - Internal.CalibrationInfo.Y0);
        Internal.Acceleration.Z =
                    ((float)Internal.Acceleration.RawZ  + (float)Internal.Acceleration.RawZLSB/2.0f - Internal.CalibrationInfo.Z0) /
    ...
    switch(Internal.ExtensionType)
        ...
        case wiimote_state::NUNCHUK:
            ...
            accel.Z = ((float)raw_z - calib.Z0) / ((float)calib.ZG - calib.Z0);
             ...
             // RIETZ: HOTFIX FOR MY CRAPPY NUNCHUCK:
            accel.Z -= 0.115;
...
//            TRACE(_T("(found extension 0x%I64x)"), type);
            ...
            static const QWORD NUNCHUK               = 0x000020A400FFULL;
            ...
            static const QWORD MOTION_PLUS_DETECT3 = 0x050020A60001ULL;
            static const QWORD PARTIALLY_INSERTED  = 0xffffffffffffULL;

            // MotionPlus: _before_ it's been activated
            if((type == MOTION_PLUS_DETECT) || (type == MOTION_PLUS_DETECT2) || (type == MOTION_PLUS_DETECT3))
                {
                if(!bMotionPlusDetected) {
            ...
            IF_TYPE(MOTION_PLUS)
                ...
                qDebug() << "Unknown extension controller:" << type;
...
bool wiimote::Load16bitMonoSampleWAV (const TCHAR* filepath, wiimote_sample &out)
    ...
    FILE *file;
#if (_MSC_VER >= 1400) // VC 2005+
    _tfopen_s(&file, (const char*)filepath, _T("rb"));
#else
...
bool wiimote::Load16BitMonoSampleRAW (const TCHAR*   filepath,
	...
	if (_tstat((const char*)filepath, &file_info)) {
